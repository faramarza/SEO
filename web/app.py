...
def api_tasks():
    ...
    for action in ledger.get_all_actions():
        ...
        tasks_by_status[column].append({
            ...
            # Metrics + scorecard + candidates for opportunity/task parity
            "recommended_action": action.action_type,
            "expected_value": round(action.score_value, 2),
            "gsc_impressions": rec.get("gsc_impressions"),
            "gsc_clicks": rec.get("gsc_clicks"),
            "gsc_ctr": rec.get("gsc_ctr"),
            "gsc_position": rec.get("gsc_position"),
            "ga4_revenue": rec.get("ga4_revenue"),
            "ga4_sessions": rec.get("ga4_sessions"),
            "ga4_engagement_rate": rec.get("ga4_engagement_rate"),
            "ga4_purchases": rec.get("ga4_purchases"),
            "ga4_bounce_rate": rec.get("ga4_bounce_rate"),
            "page_quality": rec.get("page_quality"),
...
def _persist_action(data, status=None):
    ...
    action = ActionRecord(
        ...
        recommendation_json={
            ...
            # Raw metrics + scorecard + candidates so the task view matches
            # the opportunity view (plain-English summary, metrics, badges).
            "gsc_impressions": data.get("gsc_impressions"),
            "gsc_clicks": data.get("gsc_clicks"),
            "gsc_ctr": data.get("gsc_ctr"),
            "gsc_position": data.get("gsc_position"),
            "ga4_revenue": data.get("ga4_revenue"),
            "ga4_sessions": data.get("ga4_sessions"),
            "ga4_engagement_rate": data.get("ga4_engagement_rate"),
            "ga4_purchases": data.get("ga4_purchases"),
            "ga4_bounce_rate": data.get("ga4_bounce_rate"),
